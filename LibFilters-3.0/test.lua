...
local function performFullRefresh()
	...
	LibFilters3_Test_TLW:SetHidden(true)
	LibFilters3_Test_TLW:SetHidden(false) 
end
...
local function intialize()
    ...
    local tlw = CreateTopLevelWindow("LibFilters3_Test_TLW")
	tlw:SetHidden(true)
	
	local tlc = CreateControl("LibFilters3_Test_TLC", tlw, CT_TOPLEVELCONTROL)
    ...
    local backdrop = CreateControlFromVirtual("LibFilters3_Test_TLCBg", tlc, "ZO_DefaultBackdrop")	
...
--	/lftestfilters
    ...
    local buttonshBackdrop = CreateControlFromVirtual("LibFilters3_Test_TLCbuttonsBg", buttons, "ZO_DefaultBackdrop")
...
--	LibFilters3_Test_TLC:SetHidden(true) 
end
...
local function addListDataTypes()
	...
	ZO_ScrollList_AddDataType(enableList, LIST_TYPE, "LibFilters3_Test_Template", 50, setupEnableRow)
	ZO_ScrollList_AddDataType(updateList, LIST_TYPE, "LibFilters3_Test_Template", 50, setupUpdateRow)
end
...
--[[ is there a way to check if a virtual control exists?
	...
	if not LibFilters3_Test_TLW then
		intialize()
		addListDataTypes()
	end

	if not LibFilters3_Test_TLC:IsHidden() then 
		LibFilters3_Test_TLW:SetHidden(true)
		ZO_ScrollList_Clear(enableList)
		ZO_ScrollList_Commit(enableList)
		return 
	...
	refreshEnableList(ZO_ScrollList_GetDataList(enableList))
end
